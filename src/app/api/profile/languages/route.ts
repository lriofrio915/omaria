...
export async function POST(request: Request) {
  ...
  try {
    ...
    const item = await prisma.employeeLanguage.create({
      data: {
        profileId,
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const existing = await prisma.employeeLanguage.findUnique({ where: { id } });
    if (!existing || existing.profileId !== profileId)
      return NextResponse.json({ error: "No encontrado" }, { status: 404 });
    const body = await request.json();
    const item = await prisma.employeeLanguage.update({
      where: { id },
      data: {
        language: body.language,
        level: body.level,
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    if (!id) return NextResponse.json({ error: "ID requerido" }, { status: 400 });
    const profileId = await getProfileId(user.id, user.email!);
    const item = await prisma.employeeLanguage.findUnique({ where: { id } });
    if (!item || item.profileId !== profileId)
      return NextResponse.json({ error: "No encontrado" }, { status: 404 });
    await prisma.employeeLanguage.delete({ where: { id } });
